...
- `requireTaskForEdits` defaults to **on**: a session may only change
  files while it holds a claim on a task, so "register the work first"
  is enforced instead of asked for. Observed twice in the field: a
  session opened one task, closed it, and kept editing for the rest of
  the request. Set it to false for a repository where agents should be
  free to edit anything.
- The test runner is detected from the project (jest/vitest in
  package.json, pytest in the python markers) and falls back to plain
  node, which needs nothing installed. Before, `--yes` picked jest
  unconditionally, so in a project without jest every close failed on
  "red tests" that had never run — found while writing the demo.
  The questionnaire now offers the detected runner as the Enter
  default.
- The contract states that a request rarely holds one deliverable: the
  next task is opened BEFORE the next edit, never after a close.
- The board hides `BACKLOG` and `PLANNING` until something is actually
  in them — SOLO opens work straight in `READY_FOR_DEV`, so those
  columns used to sit empty forever.
- README leads with the failure it prevents and an animated board
  (`docs/demo.gif`, regenerated by `docs/make_demo_gif.py` from a real
  run); the section that explained the principle is now the table of
